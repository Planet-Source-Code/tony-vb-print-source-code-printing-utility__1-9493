...
Function AnalyseFile(sFile As String, Optional nType) As Integer
   ...
   MdCount = MdCount + 1
   ReDim Preserve Mdl(1 To MdCount)
...
AF_ErrorHandler:
   ReportError "Problem analysing program file. Information is partly restricted"
...
Function AnalyseVBP(sVBPFile As String, frmProgress As Form) As ProjectState
   ...
   If Not FileExist(sVBPFile) Then GoTo ProjectScanAbort
   ...
   Do While Not EOF(nHandle)  ' Loop until end of file.
      ...
      Case "MODULE"
         ...
         If n > 0 Then
            ...
            If bFirstFile Then
               AnalyseVBP.StartupForm = "Sub Main() in " & AnalyseVBP.Module(AnalyseVBP.ModuleCount).Name
...
ProjectScanError:
   If bFileOpen Then Close #nHandle
   MsgBox "Encounted an access problem while scanning project file." & vbCrLf & "Error #" & Err.Number & ": " & Err.Description, vbCritical, "File Scan Error"
